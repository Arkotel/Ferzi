...
def polyline(p):
    ...
    for c in p:
        t.goto(c[0],c[1])
    t.end_fill()
'''Если последовательность p состоит, скажем, из 10 элементов,
for будет поочерёдно обходить их,
храня значение текущего элемента в переменной c. И типо таким образом мы
закрашиваем каждый элемент'''
...
def troop(x,y,z):
    ...
    circled(x+SIZE*0.5, y+SIZE*0.1, SIZE*0.4)'''
#########################################
